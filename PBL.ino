...
digitalWrite(LED1_Y, HIGH);digitalWrite(LED2_Y, HIGH);
delay(500);  
...
digitalWrite(LED4_R, LOW); digitalWrite(LED4_G, HIGH); 
digitalWrite(LED3_R, LOW);digitalWrite(LED3_G, HIGH);
delay(DelayTime); //40초간 켜줌
digitalWrite(LED4_G, LOW);digitalWrite(LED3_G, LOW);
for(int delaytime=0; delaytime <(YellowBlinkTime/1000); delaytime++){// 초록 -> 빨강 될때 황색으로 경고
digitalWrite(LED4_Y, HIGH);digitalWrite(LED3_Y, HIGH);
delay(500);  
digitalWrite(LED4_Y, LOW);digitalWrite(LED3_Y, LOW);
delay(500); 
  } 
digitalWrite(LED4_G, LOW);digitalWrite(LED3_G, LOW);  
...
for(int count=0; count<1000;count++){
 if(state == false) { //스위치가 눌렸으면
    ...
    digitalWrite(LED4_G, LOW); //초록불을 꺼준다
    digitalWrite(LED3_G, LOW); //초록불을 꺼준다
    for(int delaytime=0; delaytime <(YellowBlinkTime/1000); delaytime++){ //10초간
    digitalWrite(LED4_Y, HIGH);digitalWrite(LED3_Y, HIGH);
    delay(500);
    digitalWrite(LED4_Y, LOW);digitalWrite(LED3_Y, LOW); //노란 신호를 끔
    delay(500); //1초 간격으로 깜빡임
    } 
    digitalWrite(LED4_R, HIGH);digitalWrite(LED3_R, HIGH); //빨간 신호를 켠다
    delay(PedestrianMovingTime);
    state = true; //동작이 끝났으니 변수를 원래 상태로 돌려줌
    digitalWrite(LED4_R, LOW);digitalWrite(LED3_R, LOW);
    digitalWrite(LED4_G, HIGH);digitalWrite(LED3_G, HIGH); //다시 초록 신호를 켜준다
 }
